...
import de.kp.spark.recom.io.{ElasticBuilderFactory => EBF}
...
class RecomTracker extends BaseActor {
  ...
  private def createEvent(req:ServiceRequest) {
  ...
  private def createItem(req:ServiceRequest) {
  ...
  private def prepareEvent(params:Map[String,String]):java.util.Map[String,Object] = {
    ...
    source += EBF.EVENT_FIELD -> params(EBF.EVENT_FIELD)
 
    source
    
  }
  
  private def prepareItem(params:Map[String,String]):java.util.Map[String,Object] = {
